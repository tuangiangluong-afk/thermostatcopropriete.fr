        ...
        <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl ring-1 ring-white/10 relative z-50">
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p className="text-slate-200 text-xs">Trouvez votre expert de proximité</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSearch} className="mb-4 relative">
                <div className="relative">
                    <input
                        ...
                        className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 pl-10 text-sm focus:ring-2 focus:ring-slate-500 outline-none transition placeholder:text-slate-500 text-white"
                        autoComplete="off"
                    />
                    ...
                    <button
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-slate-500 rounded-lg hover:bg-slate-400 disabled:opacity-50 transition text-slate-950"
                    >
                        {status === 'loading' ? (
                            ...
                            {filteredSuggestions.map((s) => (
                                <li key={s.slug}>
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 hover:text-slate-700 transition flex items-center justify-between group"
                                    >
                                        <span className="font-medium text-slate-800">{s.name}</span>
                                        <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity text-slate-500" />
                                    </button>
                                </li>
                            ))}
                        </ul>
                ...
                <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            >
                                Simuler mes gains locaux <ArrowRight size={14} />
                            </Link>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="text-xs text-slate-300 mb-2">
                                Pas de site dédié pour cette zone, mais notre réseau national d&apos;installateurs QualiPAC couvre toute la France.
                            </p>
                            <Link
                                href="/#simulateur"
                                className="text-xs font-bold text-slate-400 hover:text-slate-300 underline decoration-slate-400/30"
                            >
