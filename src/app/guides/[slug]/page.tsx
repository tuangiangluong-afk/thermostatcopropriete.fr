...
const components = {
    ...
    Callout: ({ children, title }: { children: React.ReactNode, title?: string }) => (
        <div className="my-8 border-l-4 border-slate-600 bg-slate-50 p-6 rounded-r-xl">
            {title && <h4 className="font-bold text-slate-900 mb-2 flex items-center gap-2"><Zap size={18} /> {title}</h4>}
    ...
    CTAButton: ({ href, url, text }: { href?: string, url?: string, text: string }) => (
        <div className="my-8 text-center not-prose">
            <a href={href || url || '#'} className="inline-flex items-center gap-2 bg-slate-600 hover:bg-slate-700 text-white font-bold py-4 px-8 rounded-full shadow-lg hover:shadow-slate-500/30 transition-all transform hover:-translate-y-1">
...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const schema = {
        ...
        "image": guide.image ? [`https://www.thermostatcopropriete.fr${guide.image}`] : [`https://www.thermostatcopropriete.fr/api/og`],
    ...
    const howToSchema = toc.length >= 3 ? {
        ...
        "step": toc.filter((h: any) => h.level === 2).map((h: any, i: number) => ({
            ...
            "name": h.text,
            "text": h.text,
            "url": `https://www.thermostatcopropriete.fr/guides/${resolvedParams.slug}#${h.id}`
        }))
    } : null;
            ...
            <main className="container mx-auto px-4 py-12 pt-40">
                ...
                <div className="grid lg:grid-cols-[1fr_350px] gap-12 max-w-7xl mx-auto">
                    ...
                    <div>
                        {/* Header Article */}
                        <div className="mb-10">
                            <Link href="/guides" className="inline-flex items-center text-sm text-slate-500 hover:text-slate-600 mb-6 group">
                        ...
                        {/* Content Body */}
                        <article className="prose prose-lg prose-slate prose-headings:font-bold prose-headings:text-slate-900 prose-headings:scroll-mt-32 prose-a:text-slate-600 hover:prose-a:text-slate-700 prose-img:rounded-2xl max-w-none">
                            {dbPost ? (
                        ...
                        <div className="mt-16 p-8 bg-slate-50 rounded-2xl border border-slate-200">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="w-14 h-14 bg-slate-600 text-white rounded-full flex items-center justify-center text-xl font-bold shadow-md">
                                    EP
                                </div>
                                <div>
